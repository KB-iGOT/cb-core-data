...
from pyspark.sql.functions import (col, lit, coalesce, when, expr, format_string, date_format, current_timestamp,
                                   to_date,
                                   size, min as F_min, max as F_max, count as F_count, sum as F_sum, broadcast,
                                   greatest)
...
class CourseReportModel:
    ...
    def process_data(self, spark,config):
        try:
            ...
            contentHierarchyDF = (spark.read.parquet(ParquetFileConstants.CONTENT_HIERARCHY_FLATTENED_PARQUET_FILE)
                                  .withColumnRenamed("root_content_id", "courseID"))
            ...
            getContentResourceWithCategoryDF = contentHierarchyDF \
                ...
                .select(
                *[contentHierarchyDF[c] for c in contentHierarchyDF.columns],
                allCourseProgramDetailsDF["category"]
            )

            courseCategoryDF = allCourseProgramDetailsDF \
                .filter(col("courseCategory").isin(course_categories)) \
                .select(col("courseID"), col("category")) \
                .filter(col("category").isin(["Program", "Curated Program", "Course"]))

            # Level 1 = nodes whose parent_id is the courseID itself
            level1DF = contentHierarchyDF.alias("l1") \
                .join(courseCategoryDF.alias("cc"), col("l1.parent_id") == col("cc.courseID"), "inner") \
                .select(
                col("cc.courseID").alias("content_id"),
                col("cc.category").alias("category"),
                col("l1.identifier").alias("l1_identifier"),
                col("l1.name").alias("l1_name"),
                col("l1.primaryCategory").alias("l1_primaryCategory"),
                col("l1.duration").alias("l1_duration"),
                col("l1.expectedDuration").alias("l1_expectedDuration")
            )

            # Level 2 = nodes whose parent_id is a level-1 node's identifier
            level2DF = contentHierarchyDF.select(
                col("parent_id").alias("l1_identifier"),
                col("identifier").alias("l2_identifier"),
                col("name").alias("l2_name"),
                col("primaryCategory").alias("l2_primaryCategory"),
                col("duration").alias("l2_duration"),
                col("expectedDuration").alias("l2_expectedDuration")
            )

            resultDF = level1DF.join(level2DF, ["l1_identifier"], "left") \
                .select(
                col("content_id"),
                when(col("category").isin(["Program", "Curated Program"]), col("l1_identifier"))
                .otherwise(
                    when(col("l1_primaryCategory") == "Course Unit", col("l2_identifier"))
                    .otherwise(col("l1_identifier"))
                ).alias("resource_id"),
                when(col("category").isin(["Program", "Curated Program"]), col("l1_name"))
                .otherwise(
                    when(col("l1_primaryCategory") == "Course Unit", col("l2_name"))
                    .otherwise(col("l1_name"))
                ).alias("resource_name"),
                when(col("category").isin(["Program", "Curated Program"]), col("l1_primaryCategory"))
                .otherwise(
                    when(col("l1_primaryCategory") == "Course Unit", col("l2_primaryCategory"))
                    .otherwise(col("l1_primaryCategory"))
                ).alias("resource_type"),
                when(col("category").isin(["Program", "Curated Program"]), col("l1_duration"))
                .otherwise(
                    when(col("l1_primaryCategory") == "Course Unit",
                         coalesce(col("l2_duration"), col("l2_expectedDuration")))
                    .otherwise(coalesce(col("l1_duration"), col("l1_expectedDuration")))
            ...
            level1_ids = contentHierarchyDF.select(
                col("parent_id").alias("courseID"),
                col("identifier").alias("l1_identifier"),
                col("mimeType").alias("l1_mimeType")
            )

            level2_ids = contentHierarchyDF.select(
                col("parent_id").alias("l1_identifier"),
                col("mimeType").alias("l2_mimeType")
            )

            scorm_detection_df = level1_ids.join(level2_ids, ["l1_identifier"], "left") \
                .withColumn("lvl1_scorm", when(col("l1_mimeType").endswith("html-archive"), 1).otherwise(0)) \
                .withColumn("lvl2_scorm", when(col("l2_mimeType").endswith("html-archive"), 1).otherwise(0)) \
                .groupBy("courseID") \
                .agg(F_max("lvl1_scorm").alias("lvl1_flag"), F_max("lvl2_scorm").alias("lvl2_flag")) \
                .withColumn("scorm_flag", greatest(col("lvl1_flag"), col("lvl2_flag"))) \
                .select("courseID", "scorm_flag")
