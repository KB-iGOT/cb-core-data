...
from pyspark.sql.functions import (
    bround, countDistinct, sum, collect_list, col, from_json, explode_outer,
    when, expr, concat_ws, rtrim, lit, unix_timestamp, coalesce
)
...
def preComputeUser(spark: SparkSession) -> DataFrame:
    """
    Prepares and enriches user data by:
      - Reading raw user data
      - Parsing and extracting fields from the user profile JSON
      - Joining with role, karma points, and claps data
      - Writing intermediate and final user data to Parquet
    
    Args:
        spark (SparkSession): Active Spark session
    
    Returns:
        DataFrame: Final enriched user DataFrame (also exported to Parquet)
    """
...
def preComputeOrgWithHierarchy(spark: SparkSession):
    """
    Reads and prepares org and org hierarchy data, writes transformed versions to Parquet.
    """
    orgRawDF = spark.read.parquet(ParquetFileConstants.ORG_PARQUET_FILE)

    org_df = orgRawDF.select(
        col("id").alias("orgID"),
    ...
    org_hierarch_computed_df = org_hierarch_raw_df.select(
...
def preComputeOrgHierarchyWithUser(spark: SparkSession):
    """
    Joins precomputed user data with org hierarchy and exports the result.
    """
    org_merged_df = spark.read.parquet(ParquetFileConstants.ORG_COMPUTED_PARQUET_FILE + "/**.parquet")
    org_merged_df = org_merged_df.select(
        col("orgID").alias("usermergedOrgID"),
    ...
    user_merged_df = spark.read.parquet(ParquetFileConstants.USER_SELECT_PARQUET_FILE + "/**.parquet")
...
def appendContentDurationCompletionForEachUser(spark: SparkSession, user_master_df: DataFrame, user_enrolment_df: DataFrame, content_duration_df: DataFrame) -> DataFrame:
    """
    Appends total content enrolments, completions and duration (in hours) to user data.
    """
...
def timestampStringToLong(df: DataFrame, column_names: list, format: str = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") -> DataFrame:
    """
    Converts timestamp string columns to long type (epoch milliseconds).
    """
